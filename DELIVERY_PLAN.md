...
1. 继续完成 `T5 Vue 导出稳定化`
   当前重点：
   - 补齐导出态弹窗页与宿主页之间的运行时一致性
   - 检查导出态 `showDialog / closeDialog / resultPath / page state / page apis`
2. 回到 `T3 页面关系与弹窗页平台能力`
   当前重点：
   - 让详情弹窗页真正基于平台能力完成，而不是靠模板补逻辑
3. 再收尾 `T2 画布编辑与历史记录稳定化`
   当前重点：
   - 拖拽取消路径
   - 跨容器移动后的几何与索引一致性
4. 最后才进入 `T6 Demo 页面落地`
