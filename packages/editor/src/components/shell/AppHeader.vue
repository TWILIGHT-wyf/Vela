    ...
    <div class="right-section">
      ...
      <div class="action-group">
        ...
        <el-tooltip content="导出源码" placement="bottom">
          <el-button
            circle
            class="icon-btn vela-icon-btn vela-icon-btn-primary-soft"
        ...
        <el-tooltip content="导出 JSON 文件" placement="bottom">
          <el-button text circle class="icon-btn vela-icon-btn" @click="exportJSON">
...
const currentServerProjectId = computed(() =>
...
async function saveProject() {
  saving.value = true
  try {
    let message = '项目已保存到本地'
    let syncFailed = false

    if (currentServerProjectId.value) {
      try {
        await projectService.updateProject(currentServerProjectId.value, {
          name: projectStore.project.name,
          description: projectStore.project.description,
          schema: projectStore.project,
        })
        message = '项目已保存并同步到服务器'
      } catch (error) {
        syncFailed = true
    ...
    if (syncFailed) {
      ElMessage.warning('项目已保存到本地，服务器同步失败')
    } else {
...
function openPreview() {
  const previewPath = currentServerProjectId.value
    ? `/preview/${currentServerProjectId.value}`
    : '/preview'
...
.right-section {
  display: flex;
  align-items: center;
  gap: var(--space-3); /* Increased gap for breathability */
}
...
/* 按钮与图标 */
.icon-btn {
  /* 基础行为下沉到 patterns.css 的 .vela-icon-btn */
}
